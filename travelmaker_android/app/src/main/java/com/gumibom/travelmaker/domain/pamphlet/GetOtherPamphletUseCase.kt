...
class GetOtherPamphletUseCase @Inject constructor(
    private val pamphletRepositoryImpl: PamphletRepository
) {

    suspend fun getOtherPamphlet(userId : Long) : List<PamphletItem> {
        val response = pamphletRepositoryImpl.getOtherPamphlet(userId)
