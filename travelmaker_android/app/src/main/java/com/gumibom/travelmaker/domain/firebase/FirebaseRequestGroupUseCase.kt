...
class FirebaseRequestGroupUseCase @Inject constructor(
    private val firebaseFcmRepository: FirebaseFcmRepository
) {
    suspend fun requestGroup(fcmRequestGroup: FcmRequestGroupDTO)
    :BooleanResponse{

        val response = firebaseFcmRepository.groupRequest(fcmRequestGroup)
